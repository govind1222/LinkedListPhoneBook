    ...
    do {
        ...
        if (selectChar != QUIT) {
            ...
            if (valid) {
                ...
                std::cout << "The menu item '" << selectChar << "' was invalid"
                          << std::endl;
                std::cout << std::endl;
            }
        }
...
menuItem::menuItem(char selectChar, const std::string &descriptionText,
                   command &cmd) :
        theSelectChar(selectChar), theDescription(descriptionText), theCommand(
        cmd) {
}
